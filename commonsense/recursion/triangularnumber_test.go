...
func TestCalculateTriangularNumberAlt(t *testing.T) {
	tests := []struct {
		name                string
		positionToCalculate int
		want                int
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equalf(t, tt.want, CalculateTriangularNumberAlt(tt.positionToCalculate), "CalculateTriangularNumberAlt(%v)", tt.positionToCalculate)
		})
	}
}
