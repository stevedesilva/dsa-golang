...
func TestHasIncreasingTriplet(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:  "Test out of order 1",
			args:  args{prices: []float64{8, 9, 7, 10}},
			want:  true,
			want1: []float64{7, 9, 10},
		},
		{
			name:  "Test out of order 2",
			args:  args{prices: []float64{22, 25, 21, 18, 19.6, 17, 16, 20.5}},
			want:  true,
			want1: []float64{16, 19.6, 20.5},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
