...
//	 /  \    / \    / \
//	30  40  52 61  82  95
func (t *TreeNode[T]) DeleteAlt(valueToDelete T, node *TreeNode[T]) *TreeNode[T] {
	// if node to delete has no children, just delete it
	// if node to delete has one child, replace it with its child
	// if node to delete has two children, replace it with its successor
	// successor is the smallest node in the right subtree
	// replace left side of node to delete with successor
	if node == nil {
		return node
	} else if valueToDelete < node.data {
		node.left = t.DeleteAlt(valueToDelete, node.left)
		return node
	} else if valueToDelete > node.data {
		node.right = t.DeleteAlt(valueToDelete, node.right)
		return node
	} else if valueToDelete == node.data {
		if node.left == nil {
			return node.right
			...
			// lift
			node.left = t.liftAlt(node, node.right)
			return node
		}
	}
	return node
...
func (t *TreeNode[T]) liftAlt(nodeToDelete, root *TreeNode[T]) *TreeNode[T] {
	if root.left != nil {
		root.left = t.liftAlt(nodeToDelete, root.left)
		return root
	} else {
		nodeToDelete.data = root.data
		return root.right
	}
}
